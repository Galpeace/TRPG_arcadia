...
本リポジトリはGitHub MCPを通じてClaudeが直接編集できる環境で運用されています。

### 直接コミット可能な領域

Claudeは以下のフォルダ・ファイルに対して、プレイヤーの確認を得た上で直接コミットできます。

```
3_country_list/        国家・地理情報
4_character_list/      NPCリスト
6_quest_list/          クエストリスト
1_setting/1W-*         世界設定（解像度更新）
```

### Pull Request経由（プレイヤーの承認が必要）

以下のファイルを変更する場合は、必ずPull Requestを作成してプレイヤーの承認を経てからマージします。

```
2_rulebook/ 配下すべて    ルールの不変性を守るため
1_setting/1T-core_theme.md  中核思想の固定性を守るため
```

### 編集の原則

- プレイヤーの確認なしに単独でコミットしない
- コミットメッセージは `[周回N末] 変更内容` 形式で記録する
- 大規模変更は事前に変更内容をプレイヤーに提示してから実行する
- 章中（セッション進行中）の無断編集は禁止。編集は章末・周回末タイミングのみ
...
SONNETとOPUSは役割で使い分ける。切り替えのタイミングはLLMが「次は〇〇です。」と宣言するので、プレイヤーはその宣言を見て必要に応じてモデルを切り替える。

| 場面 | 推奨モデル |
|---|---|
| 章中の現場運用 | SONNET |
| 章末処理・シナリオ設計・キャラメイク | OPUS |
| 周回末アーカイブ処理 | OPUS |
...
- `3_country_list/`: 地理・国家設定
- `4_character_list/`: 重要NPC一覧
- `5_monster_list/`: モンスター一覧
- `6_quest_list/`: ワールド・クエスト一覧

これらはOPUSアーカイバが周回末にGitHub MCPを通じて直接更新します。

## 履歴

このルールブックは、LLM運用に最適化された抜本改革版です。旧B群（B1〜B12）から、構造、装飾仕様、ロール切り替え運用などを全面的に見直して構築されました。GitHub MCP導入（2026年5月）により、アーカイバによる直接更新が可能となりました。
