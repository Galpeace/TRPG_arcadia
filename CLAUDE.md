...
定型処理は `.claude/commands/` のスラッシュコマンドに集約してある。
プレイヤーがコマンドを使った場合はその指示に従い、自然文で依頼された場合も
本ファイルに基づき同じ手順を実行する。

| コマンド | 内容 |
|---|---|
| `/session-start` | 新規セッション開始（STEP 1〜4 を順に実行） |
| `/resume [日付]` | 保存済みセッションの再開（UD一式読み込み→あらすじ→続行） |
| `/chapter-end` | 章末処理（7ステップ・省略禁止） |
| `/session-end` | セッション終了処理（エンドロール→session_ud.md→保存確認） |
| `/sheet [日付]` | sheet_view.html の生成・更新 |
